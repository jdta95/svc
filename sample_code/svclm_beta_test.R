...
output = svc::svclm_beta(
  ...
  X_knots = X_knots,
  knots = knots,
  beta_knots_start = matrix(0, nrow(X_knots), ncol(X_knots)), # start with zero knots for beta
  phi_beta_start = c(phi_w, phi_1),
  sigmasq_beta_start = c(sigmasq_w, sigmasq_1),
  tausq_start = tausq,
...
beta_1_pred_plot = ggplot(data = data.frame(coords, colMeans(output$beta_samples[, , 2]))) +
  geom_tile(aes(x = lon, y = lat, fill = beta_1)) +
  guides(fill = guide_legend(title = "beta_1")) +
  coord_fixed() + 
  scico::scale_fill_scico(palette="bam") +
