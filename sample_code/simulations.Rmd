...
for(i in 1:1000)
{
 ...
 svc_fit <- spSVC(
  ...
  starting = list(
    ...
    "beta" = c(0, 3, -5)      # Added starting values for betas (intercept, beta1, beta2)
  ),
  tuning = list(
    ...
    "beta" = c(0.1, 0.1, 0.1) # Tuning for fixed effects
  ),
  priors = list(
    ...
    "beta.Norm" = list(rep(0, 3), rep(100, 3))  # Vague priors for betas
  ),
  cov.model = "exponential",
  n.samples = 5000,
  svc.cols = c(1, 2, 3)       # All coefficients have spatial variation
)
  svc_fit_rec <- spRecover(svc_fit, start = 4000, verbose = FALSE,get.beta= TRUE,get.w= TRUE)
  end_time <- Sys.time()
  ...
  bayes = rbind(bayes,c(w_means,mean(beta_samples[4000:5000,1]),mean(beta_samples[4000:5000,2]),time_taken))
}
```
