...
for (i in 1:1)
{
  ...
  one =  colMeans(low_rank$w_samples[2500:3000, , 1])
  two = colMeans(low_rank$w_samples[2500:3000, , 2])
  three = colMeans(low_rank$w_samples[2500:3000, , 3])
}
```
...
As = ggplot(data = data.frame(coords, one)) +
  geom_tile(aes(x = lon, y = lat, fill = one)) +
  coord_fixed() + 
  scico::scale_fill_scico(palette="tofino") +
  labs(x = "longitude", y = "latitude", fill = "W") +
