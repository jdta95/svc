...
Rcpp::List GP_Gibbs_beta(
  ...
  arma::mat phi_beta_samples = arma::zeros(mcmc, p) + 2;  // phi_beta samples
  arma::vec phi_w_samples = arma::zeros(mcmc) +2;        // phi_w samples
  arma::mat sigmasq_beta_samples = arma::zeros(mcmc, p) + 1; // sigma^2_beta samples
  arma::vec sigmasq_w_samples = arma::zeros(mcmc) +1;    // sigma^2_w samples
  arma::vec tausq_samples = arma::zeros(mcmc) + 1e-4;        // tau^2 samples
  ...
  for (int i = 0; i < mcmc; i++) {
    ...
    for (int j = 0; j < p; j++) {
      beta_knots_cur.col(j) = update_beta_r(Y_knots, X_knots, beta_knots_cur, w_knots_cur, knots, sigmasq_beta_cur(j), phi_beta_cur(j), tausq_cur, j);
    }
